...
export const validarActualizacionSalon = [
  param('salon_id').isInt().withMessage('El ID debe ser un número entero'),
  body('titulo').optional().isString().withMessage('El título debe ser texto'),
  body('direccion').optional().isString().withMessage('La dirección debe ser texto'),
  ...
  body('capacidad').optional().isInt({ min: 1 }).withMessage('La capacidad debe ser un número mayor que 0'),
  body('importe').optional().isFloat({ min: 0 }).withMessage('El importe debe ser un número positivo'),
...
export const validarSalonIdParam = [
  oneOf([
    param('id').isInt(),
    param('salon_id').isInt(),
  ], 'El ID del salón debe ser un número entero'),
  (req, res, next) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ ok: false, mensaje: 'Errores de validación', errores: errors.array() });
    next();
